...
if __name__ == "__main__":
    ...
    print(f"Total Number of Images: ", len(list(IMAGE_PATH.glob("*.jpg"))))

    dfx = pd.read_csv(BASE_DIR/'styles.csv', error_bad_lines=False)
    dfx = dfx.dropna()
    ...
    for i in dfx.id.tolist():
        if not os.path.isfile(os.path.join(IMAGE_PATH, str(i) + '.jpg')):
    ...
    for col in dfx.columns.tolist()[1:]:
        dfx[col] = encode_label(dfx, col)
            
    dfx.to_csv('df_final.csv', index=False)

    train_dfx, test_dfx = train_test_split(dfx, test_size=0.20, random_state=42)

    train_dfx.to_csv(BASE_DIR/'train.csv', index=False)
    test_dfx.to_csv(BASE_DIR/'test.csv', index=False)
    ...
    if 'extract_bigmodel1' in args.stage:
        ...
        fashion_dataset    = FashionImageDatasetBigModel(IMAGE_PATH=IMAGE_PATH, DF_PATH=BASE_DIR/'df_final.csv', aug = preprocess)
    ...
    if 'extract_small' in args.stage:
        ...
        model = FashionModel(num_classes=class_dict, in_features=args.input_vector_size, intermediate_features=args.intermediate_vector_size)
        model.load(os.path.join(MODEL_PATH, MODEL_NAME + "_best.pt"))
        model.to(args.device)
        model.eval()
    ...
    if 'check_vectors' in args.stage:
        ...
        dfx_train = pd.read_csv(BASE_DIR/'train.csv')
        dfx_val = pd.read_csv(BASE_DIR/'test.csv')
